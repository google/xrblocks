...
> [!NOTE]
> **three.js is a peer dependency** (`^0.186.0`). Apps load it via an importmap; never bundle
...
npm run dev            # rolldown watch + http-server on http://127.0.0.1:8080
...
> [!IMPORTANT]
> Run `npm run lint`, `npm run format`, and `npm test` before opening a PR. Tests are colocated
> `*.test.ts` files next to the code under test (Vitest, `jsdom` where DOM is needed).

`build/` is generated by Rolldown — never hand-edit it; regenerate with `npm run build:sdk`.
...
- **Build**: Rolldown ([`rolldown.config.js`](rolldown.config.js)) emits `build/xrblocks.js` (ESM),
