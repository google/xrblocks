...
The simulator exposes five values through `xb.SimulatorMode`: `USER`, `POSE`
(displayed as Navigation), `CONTROLLER` (displayed as Hands), `POINTER_LOCK`,
and `EDITOR`. Choose the initial mode before initialization:

```js
options.simulator.defaultMode = xb.SimulatorMode.CONTROLLER;
```

Keyboard mode cycling is opt-in. To use <kbd>Left Shift</kbd>:
...
Navigation/pose mode shows simulated hands while WASD/QE moves the user.
Drag the mouse to rotate the camera. Use the simulator hand-pose panel or a
configured gamepad to change poses.

### Hands Mode

Hands/controller mode uses WASD/QE to move the active hand instead of the
camera. Press <kbd>T</kbd> to switch hands and <kbd>Space</kbd> to toggle the
active hand's pinch state. Drag the mouse to rotate the active hand.

### Pointer Lock and Editor modes

Pointer Lock mode provides first-person mouse look and click selection. Editor
mode behaves like User mode but allows the optional scene editor to detect and
show its editing interface.
...
The simulator also supplies simulated planes, meshes, object detections, a
...
Physical simulated hands require Rapier through `options.physics.RAPIER`:

```js
options.simulator.handPhysics.enabled = true;
```

Set `options.simulator.physics.enabled = false` to disable simulator-owned
physics while leaving application physics available.
...
```js
{
  ...
  simulator: {
    ...
    instructions: {
      ...
      element: `xrblocks-simulator-instructions` // Name of the simulator instructions web component.
    }
  }
}
```
