...
The Start Simulator button keeps the entry UI visible while the runtime loads and initializes, with both entry buttons disabled during startup. If startup fails, it displays the error and lets the user retry. The entry UI is removed only after startup succeeds.
...
`rolldown.config.js` and `templates/14_simulator_setup/index.html`.
