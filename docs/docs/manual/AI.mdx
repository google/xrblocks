...
- **Production Deployment**: The SDK's `?key=` and `keys.json` mechanisms are for local prototypes. Production applications must configure a proxy server to forward AI requests from your client apps to Gemini endpoints. For Gemini Live, you should authenticate clients using [Gemini Live API Ephemeral Tokens](https://ai.google.dev/gemini-api/docs/live-api/ephemeral-tokens) instead of embedding long-lived developer keys.
  :::
...
// The successful result is a data:image/png;base64,... string.
if (imageDataUrl) {
  console.log('Generated image:', imageDataUrl);
}
```
