...
options.enableAI();
options.ai.promptForApiKey = true; // optional local-prototype browser prompt
...
For a local prototype, `promptForApiKey` shows a browser dialog before AI initialization.
The dialog is skipped when the URL or `keys.json` already supplies a key. The entered key
is kept only for the current page session. This prompt is not a production authentication
system.
...
const imageDataUrl = await xb.ai.generate(
...
For templates and complete projects, see <CodeSearchLink path="templates/06_ai_query/index.html"/>, <CodeSearchLink path="templates/07_ai_live_assistant/index.html"/>, or the advanced poetry sample <CodeSearchLink path="samples/xr_ai/xrpoet/index.html"/>.
