...
`enableHands()` requests `hand-tracking` as an optional WebXR feature. Browsers that cannot grant it can still enter XR and use their available controllers or screen-tap input. Hand joints and direct-touch interactions are unavailable until tracking data is present, so check returned joints before using them. An app that cannot operate without tracked hands can explicitly add `'hand-tracking'` to `options.webxrRequiredFeatures`.
