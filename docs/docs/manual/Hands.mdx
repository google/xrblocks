...
update(time, frame) {
  ...
  // Get index fingertip of the left hand as a THREE.Object3D
  const leftIndexTip = hands.getIndexTip(xb.Handedness.LEFT);

  // Get wrist of the right hand as a THREE.Object3D
  const rightWrist = hands.getWrist(xb.Handedness.RIGHT);
  ...
  const isPinching = xb.user.isSelecting(xb.Handedness.LEFT);
}
```
...
- **`xb.user.handedness`**: Value mappings:
  ...
  - `1` - Right hand only
  - `2` - Both hands active
- **`xb.Handedness`**: `LEFT`, `RIGHT`, or `NONE` (use the dominant hand).
- **`xb.HAND_JOINT_NAMES`**: The exported list of WebXR joint names.
