...
Direct touch starts the unified object-selection lifecycle by default:
`onObjectTouchStart` is followed by `onObjectSelectStart`, `onSelecting` runs
while contact continues, and `onObjectTouchEnd` is followed by
`onObjectSelectEnd`. Call `event.preventDefault()` from
`onObjectTouchStart` when contact should not select the object.
For objects with built-in manipulation, contact selects the object and pinch
starts manipulation. Releasing the pinch ends manipulation while contact can
continue.
