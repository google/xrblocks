...
meters from the viewer, Android XR's panel limits. Set
`translate: {scaleWithDistance: false, pushPull: false}` to turn those off, and
`minDistance`/`maxDistance` to change the limits. `edge: true` adds the
outer translation hit band and requires translation to be enabled. Two-source
...
By default a card never gets shorter than its content needs at the current
width, so narrowing a card makes it taller as text wraps. To let a card get
smaller than its content, put the content in a `UIScrollView` and set an
explicit `minSize.height`, which turns off the content floor:
...
const card = new xb.UICard({
  ...
  edge: true,
  manipulation: {
    actions: {translate: true, resize: {minSize: {height: 0.3}}},
  },
  children: [
    header,
    new xb.UIScrollView({
...
`minSize` defaults to 0.1 meters per axis, and `maxSize` is unbounded. A
`maxSize.height` below the content height wins over the content floor.
