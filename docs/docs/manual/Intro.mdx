...
XR Blocks is designed to be modular. Features such as GenAI, physics,
computer vision, Gaussian splats, and spatial UI use external
packages so applications only load what they need.
...
- **UI Systems**: `lit` (Simulator UI overlay), `@pmndrs/uikit` (spatial UI), `@preact/signals-core` (layout state)
...
For the main SDK, import only symbols exported by `xrblocks` (the source of
truth is `src/xrblocks.ts`). Addons use their documented entry files under
`xrblocks/addons/...`. Files that are not re-exported are internal and may
change without notice. Keep the `three` version aligned with the XR Blocks peer
dependency and do not bundle a second copy.
