...
For a starter template, see <CodeSearchLink path="templates/00_basic/index.html"/> (note that while templates in the repository use relative paths for local development, your project should use the CDN URLs below).

To use an import map, add the following script tag to your `index.html` page. This minimal
map supports the core engine. Feature templates include the extra mappings they require.
...
Install XR Blocks and its required peer dependencies with:

```bash
npm install xrblocks three @pmndrs/uikit @preact/signals-core
```

For a pre-configured template using TypeScript and Vite, check out <CodeSearchLink path="templates/13_typescript_vite/src/main.ts"/> (and its associated <CodeSearchLink path="templates/13_typescript_vite/package.json"/> and <CodeSearchLink path="templates/13_typescript_vite/tsconfig.json"/>).

#### Handling Optional Dependencies

XR Blocks keeps large providers external so applications can select the features they use.
The built-in spatial UI uses `@pmndrs/uikit` and `@preact/signals-core`; it is not a
separate XR Blocks add-on. Lit supplies the optional simulator browser interface.
...
```bash
npm install @dimforge/rapier3d @google/genai openai lit @pmndrs/uikit @preact/signals-core @mediapipe/tasks-vision @mediapipe/tasks-audio @sparkjsdev/spark three-mesh-bvh
```
...
- **Built-in spatial UI**: `@pmndrs/uikit`, `@preact/signals-core`
- **Simulator browser interface**: `lit`
...
XR Blocks uses private lazy-loaded chunks for features such as the built-in UI and simulator.
Copy and serve the complete package `build/` directory:
...
Do not deploy only `xrblocks.js`, and do not import files from `build/internal/` directly.
Those filenames are implementation details.
