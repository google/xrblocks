...
export class VoiceChat {
  ...
  async enable(currentPeers: ReadonlySet<string>): Promise<void> {
    ...
    for (const [pid, entry] of this._peers) {
      ...
      const endedSenders = senders.filter(
        (s) => s.track?.readyState === 'ended'
      );
      ...
      for (const t of stream.getTracks()) {
        ...
        if (index !== -1) {
          ...
          replacements.push(
            sender.replaceTrack(t).catch((err) => this._reportError(err, pid))
          );
        } else {
      ...
      if (addedTrack) void this._makeOffer(pid, entry);
    }
