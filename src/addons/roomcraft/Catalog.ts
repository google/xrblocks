...
function shade(base: THREE.Color, lightness: number, saturation = 1) {
  const hsl = {h: 0, s: 0, l: 0};
  base.getHSL(hsl);
  return new THREE.Color().setHSL(
    hsl.h,
    THREE.MathUtils.clamp(hsl.s * saturation, 0, 1),
    THREE.MathUtils.clamp(hsl.l * lightness, 0.03, 0.97)
  );
}

/** Derives a hue-shifted accent that still tracks the requested color. */
function accent(base: THREE.Color, hueOffset: number, lightness = 1) {
  const hsl = {h: 0, s: 0, l: 0};
  base.getHSL(hsl);
  return new THREE.Color().setHSL(
    (hsl.h + hueOffset) % 1,
    THREE.MathUtils.clamp(Math.max(hsl.s, 0.25), 0, 1),
    THREE.MathUtils.clamp(Math.max(hsl.l, 0.12) * lightness, 0.03, 0.97)
  );
}
...
function assemble(
  name: string,
  ...parts: Array<THREE.Object3D | THREE.Object3D[]>
) {
...
function createSofa(color: string) {
  ...
  return assemble(
    'sofa',
    box(1.8, 0.2, 0.85, body, 0, 0.28),
    box(1.8, 0.47, 0.16, body, 0, 0.615, -0.345),
    box(0.16, 0.28, 0.85, body, -0.82, 0.52),
    box(0.16, 0.28, 0.85, body, 0.82, 0.52),
    box(0.78, 0.14, 0.7, cushion, -0.42, 0.45, 0.03),
    box(0.78, 0.14, 0.7, cushion, 0.42, 0.45, 0.03),
    box(0.76, 0.3, 0.08, cushion, -0.42, 0.6, -0.22),
    box(0.76, 0.3, 0.08, cushion, 0.42, 0.6, -0.22),
    scatter(new THREE.CylinderGeometry(0.045, 0.035, 0.18, 10), leg, [
      [-0.75, 0.09, 0.33],
      [0.75, 0.09, 0.33],
      [-0.75, 0.09, -0.33],
      [0.75, 0.09, -0.33],
...
function createArmchair(color: string) {
  const base = new THREE.Color(color);
  const body = material(base, 0.85);
  const cushion = material(shade(base, 1.25, 0.9), 0.9);
  const leg = material(shade(base, 0.35, 0.6), 0.5, 0.25);
  return assemble(
    'armchair',
    box(0.8, 0.2, 0.8, body, 0, 0.28),
    box(0.8, 0.52, 0.16, body, 0, 0.64, -0.32),
    box(0.13, 0.3, 0.72, body, -0.335, 0.53, 0.02),
    box(0.13, 0.3, 0.72, body, 0.335, 0.53, 0.02),
    box(0.6, 0.14, 0.66, cushion, 0, 0.45, 0.02),
    box(0.56, 0.32, 0.08, cushion, 0, 0.62, -0.2),
    scatter(new THREE.CylinderGeometry(0.04, 0.03, 0.18, 10), leg, [
      [-0.3, 0.09, 0.28],
      [0.3, 0.09, 0.28],
      [-0.3, 0.09, -0.28],
      [0.3, 0.09, -0.28],
...
function createCoffeeTable(color: string) {
  ...
  return assemble(
    'coffee-table',
    box(1, 0.05, 0.55, top, 0, 0.425),
    box(0.9, 0.04, 0.45, trim, 0, 0.38),
    box(0.85, 0.03, 0.42, top, 0, 0.14),
    scatter(new THREE.BoxGeometry(0.06, 0.4, 0.06), trim, [
      [-0.45, 0.2, 0.235],
      [0.45, 0.2, 0.235],
      [-0.45, 0.2, -0.235],
      [0.45, 0.2, -0.235],
...
function createBookshelf(color: string) {
  ...
  for (const shelfY of [0.05, 0.48, 0.91, 1.34]) {
    ...
    for (let index = 0; index < 5 && x < 0.36; index++) {
      ...
      books.push(
        box(width, height, 0.2, spine, x + width / 2, shelfY + height / 2, 0.02)
      );
  ...
  return assemble(
    'bookshelf',
    box(1.1, 1.8, 0.03, panel, 0, 0.9, -0.145),
    box(0.05, 1.8, 0.32, frame, -0.525, 0.9),
    box(0.05, 1.8, 0.32, frame, 0.525, 0.9),
    box(1, 0.05, 0.3, frame, 0, 0.025, 0.01),
    box(1, 0.05, 0.3, frame, 0, 1.775, 0.01),
    box(1, 0.04, 0.3, frame, 0, 0.46, 0.01),
    box(1, 0.04, 0.3, frame, 0, 0.89, 0.01),
    box(1, 0.04, 0.3, frame, 0, 1.32, 0.01),
    books
  );
}
...
function createFloorLamp(color: string) {
  ...
  const shadeMesh = new THREE.Mesh(
    new THREE.CylinderGeometry(0.15, 0.25, 0.34, 20, 1, true),
    shadeMaterial
  );
  ...
  return assemble(
    'floor-lamp',
    tube(0.25, 0.25, 0.04, metal, 0, 0.02, 0, 20),
    tube(0.02, 0.028, 1.4, metal, 0, 0.7, 0, 10),
    shadeMesh,
    bulb
  );
}

function createPlant(color: string) {
  const base = new THREE.Color(color);
  ...
  return assemble(
    'plant',
    tube(0.19, 0.14, 0.26, pot, 0, 0.13, 0, 14),
    tube(0.2, 0.2, 0.04, rim, 0, 0.26, 0, 14),
    tube(0.175, 0.175, 0.02, soil, 0, 0.26, 0, 14),
    tube(0.014, 0.018, 0.42, stem, 0, 0.46, 0, 8),
    leaves
  );
}

function createPlinth(color: string) {
  const base = new THREE.Color(color);
  ...
  return assemble(
    'plinth',
    box(0.45, 0.06, 0.45, foot, 0, 0.03),
    box(0.36, 0.72, 0.36, column, 0, 0.42),
    box(0.45, 0.05, 0.45, cap, 0, 0.825)
  );
}

function createArtPanel(color: string) {
  const base = new THREE.Color(color);
  ...
  const artwork: THREE.Mesh[] = shapes.map(({x, y, width, height, tone}) =>
    box(width, height, 0.02, relief[tone], x, 0.8 + y, 0.045)
  );
  ...
  return assemble(
    'art-panel',
    box(0.5, 0.05, 0.16, stand, 0, 0.025),
    box(0.04, 0.32, 0.04, stand, -0.18, 0.2),
    box(0.04, 0.32, 0.04, stand, 0.18, 0.2),
    box(0.75, 0.9, 0.05, frame, 0, 0.8),
    box(0.67, 0.82, 0.02, canvasMaterial, 0, 0.8, 0.03),
    artwork
  );
}

function createArch(color: string) {
  const base = new THREE.Color(color);
  ...
  return assemble(
    'arch',
    body,
    box(0.22, 0.24, 0.3, detail, 0, 2.28),
    box(0.5, 0.12, 0.3, detail, -0.95, 0.06),
    box(0.5, 0.12, 0.3, detail, 0.95, 0.06)
  );
}

function createBuilding(color: string) {
  const base = new THREE.Color(color);
  ...
  const windows = scatter(
    new THREE.BoxGeometry(0.06, 0.08, 0.006),
    glass,
    offsets
  );
  return assemble(
    'building',
    box(0.32, 0.6, 0.32, facade, 0, 0.3),
    box(0.35, 0.04, 0.35, trim, 0, 0.62),
    box(0.22, 0.14, 0.22, facade, 0, 0.71),
    box(0.24, 0.02, 0.24, trim, 0, 0.79),
    box(0.08, 0.12, 0.006, door, 0, 0.06, 0.161),
    windows
  );
}

function createTree(color: string) {
  const base = new THREE.Color(color);
  ...
  return assemble(
    'tree',
    tube(0.022, 0.032, 0.18, trunk, 0, 0.09, 0, 8),
    new THREE.Mesh(new THREE.ConeGeometry(0.125, 0.2, 7), canopy).translateY(
      0.24
    ),
    new THREE.Mesh(new THREE.ConeGeometry(0.1, 0.17, 7), canopy).translateY(
      0.34
    ),
    new THREE.Mesh(new THREE.ConeGeometry(0.07, 0.14, 7), upper).translateY(
      0.43
    )
  );
}
...
function createSphere(color: string) {
  const mesh = new THREE.Mesh(
...
function createCylinder(color: string) {
  return assemble(
    'cylinder',
    tube(0.2, 0.2, 0.7, material(color, 0.7), 0, 0.35, 0, 24)
  );
}

function createCone(color: string) {
  const mesh = new THREE.Mesh(
 ...
 */
export function createDefaultCatalog(): SceneAsset[] {
  return [
    {
      id: 'sofa',
...
function tint(object: THREE.Object3D, color: THREE.Color) {
  ...
  object.traverse((child) => {
    ...
    for (const item of materials) {
      ...
      const tinted = item as THREE.Material & {color?: unknown};
