...
interface EnvironmentPalette {
  /** Sky gradient from the horizon band up to the zenith. */
  zenith: string;
  horizon: string;
  /** Distant haze below the horizon, visible past the ground edge. */
  haze: string;
  /** The sun or moon disc, its halo, and how tightly that halo falls off. */
  body: string;
  bodyElevation: number;
  bodyAzimuth: number;
  bodyAngularRadius: number;
  /** 1 shades the disc like a moon; 0 leaves a clean solar disc. */
  bodyDetail: number;
  glow: string;
  glowStrength: number;
  glowFalloff: number;
  ...
  /** Multiplies the authored ground color into this time of day. */
  groundTint: string;
  groundRoughness: number;
}

const PALETTES: Record<SceneTimeOfDay, EnvironmentPalette> = {
  moonlight: {
    zenith: '#050a1c',
    horizon: '#16274f',
    haze: '#070b16',
...
function readPalette(environment: SceneEnvironment): EnvironmentPalette {
  const palette = SCENE_TIMES_OF_DAY.includes(environment.timeOfDay)
    ? PALETTES[environment.timeOfDay]
    : undefined;
  if (!palette) {
    throw new Error(
      `Unsupported environment time of day "${environment.timeOfDay}".`
    );
  }
  return palette;
}

function readSize(environment: SceneEnvironment): [number, number] {
  ...
  if (
    ...
    throw new Error(
      'A virtual environment needs a finite, positive ground size.'
    );
  }
  return [size[0], size[1]];
}

function directionOf(elevation: number, azimuth: number) {
  const polar = THREE.MathUtils.degToRad(elevation);
  const around = THREE.MathUtils.degToRad(azimuth);
  return new THREE.Vector3(
    Math.cos(polar) * Math.sin(around),
    Math.sin(polar),
    -Math.cos(polar) * Math.cos(around)
  );
}
 ...
 */
function createGroundGeometry(width: number, depth: number) {
  ...
  for (let index = 0; index < position.count; index++) {
    ...
    const shade =
      (1 - EDGE_SHADE * THREE.MathUtils.smoothstep(rim, 0.65, 1)) *
 ...
 *     at Y=0.
 */
export function getEnvironmentBounds(
  ...
  return new THREE.Box3(
    new THREE.Vector3(-width / 2, -GROUND_THICKNESS, -depth / 2),
 ...
 */
export function createEnvironmentContent(
  environment: SceneEnvironment
): THREE.Group {
  const [width, depth] = readSize(environment);
  ...
  try {
    ...
    const groundMaterial = new THREE.MeshStandardMaterial({
      color: new THREE.Color(environment.groundColor).multiply(
        new THREE.Color(palette.groundTint)
      ),
    ...
    const bodyDirection = directionOf(
      palette.bodyElevation,
      palette.bodyAzimuth
    );
    ...
    key.position
      .copy(
        directionOf(
          Math.max(palette.bodyElevation, MIN_KEY_ELEVATION),
          palette.bodyAzimuth
        )
      )
    ...
    const extent = THREE.MathUtils.clamp(
      Math.hypot(width, depth) / 2 + SHADOW_MARGIN,
      MIN_SHADOW_EXTENT,
      MAX_SHADOW_EXTENT
    );
