...
function track<T extends Disposable>(owned: Disposable[], resource: T): T {
  owned.push(resource);
  return resource;
}
...
function assertFinite(values: readonly number[], message: string) {
...
function readVector2(value: unknown, message: string): [number, number] {
  if (!Array.isArray(value) || value.length !== 2) throw new Error(message);
  assertFinite(value as number[], message);
  return [value[0] as number, value[1] as number];
}

function assertPond(definition: ScenePond) {
  const [width, depth] = readVector2(
    definition.size,
    'A pond needs a finite water width and depth.'
  );
  ...
  if (
    ...
    throw new Error(
      `A pond needs a water size up to ${MAX_LANDSCAPE_SIZE} meters.`
    );
  }
  if (definition.bankWidth <= 0 || definition.bankWidth > MAX_BANK_WIDTH) {
    throw new Error(
      `A pond bank must be wider than 0 and at most ${MAX_BANK_WIDTH} meters.`
    );
  }
}

function assertPath(definition: ScenePath) {
  ...
  if (
    ...
    throw new Error(
      `A path needs ${MIN_PATH_POINTS} to ${MAX_PATH_POINTS} points.`
    );
  }
  assertFinite([definition.width], 'A path needs a finite width.');
  if (definition.width <= 0 || definition.width > MAX_PATH_WIDTH) {
    throw new Error(
      `A path must be wider than 0 and at most ${MAX_PATH_WIDTH} meters.`
    );
  }
  ...
  for (const point of points) {
    ...
    if (Math.abs(x) > MAX_SCENE_DISTANCE || Math.abs(z) > MAX_SCENE_DISTANCE) {
      throw new Error(
        `A path stays within ${MAX_SCENE_DISTANCE} meters of its origin.`
      );
    }
    if (
      previous &&
      ...
      throw new Error(
        `A path needs adjacent points at least ${MIN_PATH_SEGMENT} meters apart.`
      );
    }
    previous = [x, z];
  }
...
function assertScatter(definition: SceneScatter) {
  ...
  if (
    ...
    throw new Error(
      `A scatter needs a planting area up to ${MAX_LANDSCAPE_SIZE} meters.`
    );
  }
  if (definition.height <= 0 || definition.height > MAX_SCATTER_HEIGHT) {
    throw new Error(
      `A scatter specimen must be taller than 0 and at most ${MAX_SCATTER_HEIGHT} meters.`
    );
  }
  if (
    ...
    throw new Error(
      `A scatter needs a whole count from 1 to ${MAX_SCATTER_COUNT}.`
    );
  }
  if (
    !Number.isInteger(definition.seed) ||
    definition.seed < 0 ||
    definition.seed > MAX_SCATTER_SEED
  ) {
    throw new Error(
      `A scatter needs a whole seed from 0 to ${MAX_SCATTER_SEED}.`
    );
  }
}

/** Validates one recipe before it is measured or built. */
function assertLandscape(definition: SceneLandscape) {
  ...
  switch (definition.kind) {
    ...
    default:
      throw new Error(
        `Unsupported landscape kind "${(definition as {kind: string}).kind}".`
      );
  }
}
...
function blend(base: THREE.Color, toward: string, amount: number, shade = 1) {
  return clampColor(
    base.clone().lerp(new THREE.Color(toward), amount).multiplyScalar(shade)
  );
}
 ...
 */
function createBlobGeometry(owned: Disposable[], variant: number) {
  const geometry = track(
    owned,
    new THREE.IcosahedronGeometry(0.5, BLOB_DETAIL)
  );
...
function speckle(count: number, tone: (index: number) => number) {
...
function rimStoneHalfExtent(size: number, tilt: number) {
...
function rimStoneExtent(bankWidth: number) {
  const half = rimStoneHalfExtent(
    RIM_STONE_MAX_SIZE * bankWidth,
    RIM_STONE_TILT
  );
  return {
...
function getPondBounds(definition: ScenePond) {
  ...
  const top = Math.max(
    POND_WATER_Y + RIPPLE_AMPLITUDE,
    POND_WATER_Y + BANK_CREST_RISE * definition.bankWidth,
    stones.top
  );
  return new THREE.Box3(
    new THREE.Vector3(-width / 2 - reach, 0, -depth / 2 - reach),
    new THREE.Vector3(width / 2 + reach, top, depth / 2 + reach)
  );
}
...
function createBankGeometry(
  ...
  geometry.setAttribute(
    'color',
    speckle(rings.length * BANK_SEGMENTS, (index) =>
      Math.min(1, rings[Math.floor(index / BANK_SEGMENTS)].tone)
    )
  );
  geometry.computeVertexNormals();
  return geometry;
...
function buildPond(
  ...
  const bed = new THREE.Mesh(
    track(
      owned,
      new THREE.CircleGeometry(0.5, BED_SEGMENTS)
        .rotateX(-Math.PI / 2)
        .scale(definition.size[0], 1, definition.size[1])
        .translate(0, POND_BED_Y, 0)
    ),
    track(
      owned,
  ...
  const water = new THREE.Mesh(
    createWaterGeometry(owned, radiusX, radiusZ),
    track(
      owned,
  ...
  const bank = new THREE.Mesh(
    createBankGeometry(owned, radiusX, radiusZ, bankWidth),
    track(
      owned,
      new THREE.MeshStandardMaterial({
  ...
  for (let index = 0; index < count; index++) {
    const angle =
      ((index + hashUnit(RIM_STONE_SEED, index, CHANNEL_YAW) * 0.7) / count) *
      Math.PI *
      2;
    const offset =
      bankWidth *
    ...
    const lean =
      hashUnit(RIM_STONE_SEED, index, CHANNEL_LEAN_AXIS) * Math.PI * 2;
    ellipseNormal(radiusX, radiusZ, angle, normal);
    ...
    position.set(
      radiusX * Math.cos(angle) + normal.x * offset,
      rimStoneHalfExtent(size, tilt).up,
      radiusZ * Math.sin(angle) + normal.y * offset
    );
    quaternion
      ...
      .multiply(
        new THREE.Quaternion().setFromAxisAngle(
          UP,
          hashUnit(RIM_STONE_SEED, index, CHANNEL_Z) * Math.PI * 2
        )
      );
...
/** A sampled center line with unit left-hand normals, one per sample. */
interface PathStrip {
 ...
 */
function samplePathStrip(definition: ScenePath): PathStrip {
  ...
  for (const point of curve.getPoints(divisions)) {
    ...
    if (previous && previous.distanceTo(sample) < MIN_SAMPLE_SPACING) continue;
    center.push(sample);
  }
  if (center.length < 2) {
 ...
 */
function stripEdges(strip: PathStrip, halfWidth: number, extend: number) {
  ...
  if (extend > 0) {
    ...
    center.unshift(
      new THREE.Vector2(
        center[0].x - head.y * extend,
        center[0].y + head.x * extend
      )
    );
    normal.unshift(head.clone());
    const last = center[center.length - 1];
    center.push(
      new THREE.Vector2(last.x + tail.y * extend, last.y - tail.x * extend)
    );
...
function createStripGeometry(
  ...
  geometry.setAttribute(
    'color',
    speckle(left.length * 2, () => 1)
  );
  return geometry;
}

function getPathBounds(definition: ScenePath) {
  const strip = samplePathStrip(definition);
  const shoulder = pathShoulderWidth(definition.width);
  const {left, right} = stripEdges(
    strip,
    definition.width / 2 + shoulder,
    shoulder
  );
...
function buildPath(
  ...
  const surface = new THREE.Mesh(
    createStripGeometry(owned, strip, definition.width / 2, 0, PATH_SURFACE_Y),
    track(
      owned,
      new THREE.MeshStandardMaterial({
        color: primary.clone(),
...
type ScatterShape = 'blob' | 'column' | 'blade';
type ScatterPalette = 'primary' | 'wood' | 'stem';

/** One instanced component of a specimen, in fractions of its height. */
interface ScatterPart {
  name: string;
  shape: ScatterShape;
  palette: ScatterPalette;
  /** Height of the component center above the specimen base. */
  center: number;
  /** Half the component's vertical extent. */
  halfHeight: number;
  /** Horizontal radius of the component. */
  radius: number;
  /** Largest lateral offset from the specimen axis. */
  jitter: number;
  /** Outward tilt about the component center, in radians. */
  spread?: number;
  /** Brightness relative to the palette color. */
  shade?: number;
  flat?: boolean;
}

interface ScatterProfile {
  /** Largest whole-specimen lean about its base, in radians. */
  lean: number;
  /** Shortest specimen as a fraction of the authored height. */
  minHeight: number;
  parts: ScatterPart[];
}
 ...
 */
const SCATTER_PROFILES: Record<SceneScatterStyle, ScatterProfile> = {
  tree: {
    lean: 0.08,
    minHeight: 0.7,
    parts: [
...
function scatterPartExtent(profile: ScatterProfile, part: ScatterPart) {
  const spread = part.spread ?? 0;
  const halfHeight =
    part.halfHeight * Math.cos(spread) + part.radius * Math.sin(spread);
  const radius =
    part.radius * Math.cos(spread) + part.halfHeight * Math.sin(spread);
...
function getScatterBounds(definition: SceneScatter) {
  ...
  return new THREE.Box3(
    new THREE.Vector3(
      -definition.size[0] / 2 - overhang,
      bottom * height,
      -definition.size[1] / 2 - overhang
    ),
    new THREE.Vector3(
      definition.size[0] / 2 + overhang,
      top * height,
...
function createScatterGeometry(
  owned: Disposable[],
  part: ScatterPart,
  variant: number
) {
  switch (part.shape) {
    ...
    case 'column':
      return track(
        owned,
        new THREE.CylinderGeometry(0.32, 0.5, 1, COLUMN_SEGMENTS)
      );
    case 'blade':
...
function buildScatter(
  ...
  const specimens: Array<{matrix: THREE.Matrix4; height: number}> = [];
  for (let index = 0; index < count; index++) {
    const lean = profile.lean * hashUnit(seed, index, CHANNEL_LEAN) ** 2;
    const leanAxis = hashUnit(seed, index, CHANNEL_LEAN_AXIS) * Math.PI * 2;
    position.set(
      (hashUnit(seed, index, CHANNEL_X) - 0.5) * areaWidth,
      0,
      (hashUnit(seed, index, CHANNEL_Z) - 0.5) * areaDepth
    );
    quaternion
      .setFromAxisAngle(
        axis.set(Math.sin(leanAxis), 0, -Math.cos(leanAxis)),
        lean
      )
      .multiply(
        spin.setFromAxisAngle(
          UP,
          hashUnit(seed, index, CHANNEL_YAW) * Math.PI * 2
        )
      );
    specimens.push({
  ...
  for (const [partIndex, part] of profile.parts.entries()) {
    const mesh = track(
      owned,
      new THREE.InstancedMesh(
        createScatterGeometry(owned, part, partIndex + 1),
        track(
          owned,
          new THREE.MeshStandardMaterial({
    ...
    for (let index = 0; index < count; index++) {
      ...
      quaternion
        .setFromAxisAngle(
          axis.set(Math.sin(jitterAngle), 0, -Math.cos(jitterAngle)),
          part.spread ?? 0
        )
        .multiply(
          spin.setFromAxisAngle(
            UP,
            hashUnit(seed, index, channel + 2) * Math.PI * 2
          )
        );
      scale.set(
        part.radius * 2 * size,
        part.halfHeight * 2 * size,
        part.radius * 2 * size
      );
      local.compose(position, quaternion, scale);
      mesh.setMatrixAt(
        index,
        instance.multiplyMatrices(specimen.matrix, local)
      );
 ...
 */
export function getLandscapeBounds(definition: SceneLandscape): THREE.Box3 {
 ...
 */
export function createLandscapeContent(
  definition: SceneLandscape,
  color: string
): THREE.Group {
  assertLandscape(definition);
