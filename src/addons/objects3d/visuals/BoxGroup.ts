 ...
 * light = yellow.
 */
export const CAT_COLOR: Record<string, number> = {
  flat: 0xff9f0a,
  furniture: 0x4cd964,
  small: 0x64d2ff,
  light: 0xffd60a,
 ...
 */
export function makeLabelSprite(text: string, lineColor: number): THREE.Sprite {
  const padX = 24;
  const padY = 14;
  const fontPx = 64;
...
export function buildBoxGroup(
  ...
  (group.userData as {label: THREE.Sprite; labelText: string}).label =
    labelSprite;
  (group.userData as {labelText: string}).labelText = label;
 ...
 */
export function rebuildBoxGroupGeometry(
  group: THREE.Group,
  obb: InternalObb
): void {
  ...
  group.position.copy(obb.center);
  group.rotation.y = obb.angle;
  const ud = group.userData as {label?: THREE.Sprite};
  if (ud.label) {
