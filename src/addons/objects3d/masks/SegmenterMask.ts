...
// @mediapipe/tasks-vision is an external peer-dep loaded via importmap at
// runtime. Define an opaque local type so TypeScript doesn't require the
// package to be installed in node_modules.
...
let _segmenter: InteractiveSegmenter | null = null;
let _segmenterPromise: Promise<InteractiveSegmenter> | null = null;
 ...
 */
export function getSegmenter(): Promise<InteractiveSegmenter> {
  if (_segmenter) return Promise.resolve(_segmenter);
  if (_segmenterPromise) return _segmenterPromise;
  _segmenterPromise = (async () => {
    const {FilesetResolver, InteractiveSegmenter: ISCls} = await import(
 ...
 */
export async function segmenterMaskFromSnapshot(
  snapshot: ImageData,
      ...
      (
        seg as unknown as {
          segment(
            ...
            opts: {keypoint: {x: number; y: number}},
