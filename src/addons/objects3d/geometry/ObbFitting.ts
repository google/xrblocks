...
function pctile(arr: number[], q: number): number {
  ...
  return sorted[
    Math.max(0, Math.min(sorted.length - 1, Math.floor(q * sorted.length)))
  ];
}
 ...
 */
export function rejectByAnchor(
  points: THREE.Vector3[],
...
export function radiusFromBbox(
  box2d: THREE.Box2 | null | undefined,
  ...
  anchor: THREE.Vector3 | null | undefined,
  {
    ...
    maxR = 2.0,
    fallback = 0.8,
  }: {pad?: number; minR?: number; maxR?: number; fallback?: number} = {}
): number {
 ...
 */
export function rejectByAnchorDepth(
  points: THREE.Vector3[],
  camera: THREE.PerspectiveCamera | null | undefined,
  anchor: THREE.Vector3 | null | undefined,
  tol: number
...
export function ransacPlane(
  ...
  for (let it = 0; it < iters; it++) {
    ...
    let count = 0;
    for (const p of points) {
      if (Math.abs(_n.dot(p) + d) < eps) count++;
    }
  ...
  if (!best.normal || !best.point || best.inliers.length < 6) return null;
  const centroid = new THREE.Vector3();
  for (const p of best.inliers) centroid.add(p);
 ...
 */
export function projectBboxToPlane(
  box2d: THREE.Box2,
  camera: THREE.PerspectiveCamera,
  planePoint: THREE.Vector3,
  planeNormal: THREE.Vector3
 ...
 * @returns Fitted {@link InternalObb}, or `null` on failure.
 */
export function fitTinyFlatOBB(opts: ObbFitOptions): InternalObb | null {
  ...
  const ud = cam.userData as {snapAspect?: number};
  ...
  let best: {t: number; hit: THREE.Vector3; n: THREE.Vector3} | null = null;
