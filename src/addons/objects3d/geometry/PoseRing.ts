...
export class PoseRing {
  ...
  velocityAround(
    t: number,
    halfWindowMs = 90
  ): {linearMetersPerSec: number; angularRadPerSec: number} | null {
    let earliest: {t: number; worldFromView: THREE.Matrix4} | null = null;
    let latest: {t: number; worldFromView: THREE.Matrix4} | null = null;
    for (let i = 0; i < this.count; ++i) {
      const entry = this.entries[i];
      if (Math.abs(entry.t - t) > halfWindowMs) continue;
      if (!earliest || entry.t < earliest.t) earliest = entry;
      if (!latest || entry.t > latest.t) latest = entry;
    }
    if (!earliest || !latest) return null;
    const dtSeconds = (latest.t - earliest.t) / 1000;
    if (dtSeconds < 0.02) return null;
    const positionA = new THREE.Vector3().setFromMatrixPosition(
      earliest.worldFromView
    );
    const positionB = new THREE.Vector3().setFromMatrixPosition(
      latest.worldFromView
    );
    const quaternionA = new THREE.Quaternion().setFromRotationMatrix(
      earliest.worldFromView
    );
    const quaternionB = new THREE.Quaternion().setFromRotationMatrix(
      latest.worldFromView
    );
    return {
